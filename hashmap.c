...
long hashFunction(const char *str_key, hashmap *hash) {
  int len, i, size;
  long value;
  ...
  len = strlen(str_key);
  for (i = 0; i < len; i++) {
    value = value + ((int) str_key[i]) * pow(31, len-1-i);
    value %= size; // do modular operation for each iteration
  }
...
void printHashTable(hashmap *hash) {
  ...
  for (i = 0; i < hash->size; i++) {
    if (hash->table[i].flags & ACTIVE) {
        printf("key:%ld    value:%s\n", hash->table[i].key, (char *)hash->table[i].data);
    }
  }
}
